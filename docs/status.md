...
| **Authentication boundary**: host-supplied `AdminAuth`, required, protects every route incl. `/admin/meta` | `packages/nestjs/src/auth`             |
| **401/403 as distinct outcomes**, mapped from Core `UnauthorizedError` / `ForbiddenError`                  | `packages/core/src/errors`             |
| **Resource-level authorization**: per-model, per-operation, enforced in one place                          | `packages/nestjs/src/auth/resource.ts` |
| **Metadata filtering**: `/admin/meta` describes only the resources the principal may see                   | `packages/nestjs/src/admin/service.ts` |
| **229 tests**, integration against real SQLite and a real Nest HTTP server                                 | `packages/*/test`, `tests/`            |
...
- Field-level permissions (per-model policy exists; per-field does not)
- RBAC, roles, a permission store or policy DSL - the host owns the decision
- Any login, session, JWT or user storage - by design, the host application
  owns identity
