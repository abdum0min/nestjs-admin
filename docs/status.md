...
| **183 tests**, integration against real SQLite and a real Nest HTTP server                                 | `packages/*/test`, `tests/`           |
...
- Resource-level permissions, RBAC, roles (the auth boundary exists; per-model
  and per-field policy does not)
